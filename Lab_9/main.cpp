...
void saveToBinary(const Route arr[], int size, const char* filename) {
    ...
    out.write((char*)arr, sizeof(Route) * size);
    out.close();
    cout << "Данные сохранены в файл: " << filename << endl;
}

int loadFromBinary(Route arr[], int maxSize, const char* filename) {
    ...
    if (size > maxSize) {
        cout << "Ошибка: данных в файле больше, чем вмещает массив" << endl;
        in.close();
        return 0;
    }
    in.read((char*)arr, sizeof(Route) * size);
    in.close();
    cout << "Загружено " << size << " записей из файла" << endl;
    return size;
}

void updateFromFile(Route arr[], int size, const char* filename) {
    ifstream fin(filename);
    if (!fin.is_open()) {
        cout << "Файл обновлений не найден: " << filename << endl;
        return;
    }
    char name[50];
    int newWagons;
    int count = 0;
    while (fin >> name >> newWagons) {
        for (int i = 0; i < size; ++i) {
            if (strcmp(arr[i].nameRoute, name) == 0) {
                arr[i].wagonCount = newWagons;
                cout << "Обновлено: " << name << " -> " << newWagons << " вагонов" << endl;
                count++;
                break;
            }
        }
    ...
    fin.close();
    if (count == 0) cout << "Ни одна запись не обновлена" << endl;
}
...
int main() {
    ...
    saveToBinary(allRoutes, SIZE, "routes.bin");
    ofstream upd("updates.txt");
    upd << "Ласточка-Юг 25" << endl;
    upd << "Ока 12" << endl;
    upd.close();
    cout << "\n--- Обновление из файла updates.txt ---" << endl;
    updateFromFile(allRoutes, SIZE, "updates.txt");
    ...
    cout << "\n--- Проверка чтения из routes.bin ---" << endl;
    Route loaded[SIZE];
    int loadedCount = loadFromBinary(loaded, SIZE, "routes.bin");
    if (loadedCount > 0) {
        cout << "Первая запись: " << loaded[0].nameRoute << endl;
    }
    return 0;
}
